...
generate_proposal(data_path='./test0/',save_path='./test/',resize_h_w_ratio=0.6)
...
with tf.Session() as sess:
    ...
    for num in range(folds_num):
        ...
        for key in image_tensor:
            ...
            name = key.split('p')[0]+'.jpg'
            if name not in img_dict.keys():
                img_dict[name] = results
            else:
                temp_max = max(img_dict[name], key=img_dict[name].get)
                this_max = max(results, key=results.get)
                if results[this_max] > img_dict[name][temp_max]:
                    img_dict[name] = results
